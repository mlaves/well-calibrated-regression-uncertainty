...
from calibration import Scaler
...
def train(base_model,
    ...
    if dataset == 'breastpathq':
        ...
        train_indices = torch.load(f'./{dataset}_train_indices.pth')
        valid_indices = torch.load(f'./{dataset}_valid_indices.pth')
    ...
    scaler = Scaler(init_S=1.0).to(device)
        ...
        for e in range(epochs):
            ...
            with torch.no_grad():
                for batch_idx, (data, targets) in enumerate(tqdm(valid_loader)):
                    ...
                    uncert_calib_aleatoric = logvar.exp()
                    uncert_calib_epistemic = var_bayesian
                    ...
                    uncert_calib = scaler(uncert_calib)**2
                    logvar_valid.append(uncert_calib.cpu())
                    ...
                    writer.add_scalar('valid/var', uncert_calib.mean(), batch_counter_valid)
